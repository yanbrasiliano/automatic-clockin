...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text.lower() in ["yes", "y"]:
        ...
        screenshot_file = os.path.join(SCREENSHOT_PATH, 'screenshot_after_register.png')
        ...
        if os.path.exists(screenshot_file):
            ...
            print(f"Sent screenshot after register to {update.effective_chat.id}")
        else:
        ...
        screenshot_file = os.path.join(SCREENSHOT_PATH, 'screenshot_after_login.png')
        print(f"Looking for screenshot: {screenshot_file}")
        
        # Take a screenshot (assuming your script can handle taking a screenshot without performing other actions)
        result = subprocess.run(['python3', SCRIPT_PATH, '--screenshot'], capture_output=True, text=True)
        ...
        if os.path.exists(screenshot_file):
            await context.bot.send_photo(chat_id=update.effective_chat.id, photo=open(screenshot_file, 'rb'))
            print(f"Sent current page screenshot to {update.effective_chat.id}")
        else:
            await update.message.reply_text('Screenshot not found.')
